...
test('same-origin client bridge forwards API controls and handles failures', { timeout: 40_000 }, async (t) => {
	...
	assert.equal((await put('settings', { mode: 'season' })).status, 204);
	assert.equal((await (await fetch(`${base}/api/settings`)).json()).mode, 'season');
	assert.equal(mock.state.shows[1].mode, 'season');
	assert.equal((await put('settings', { mode: 'episode' })).status, 204);
	assert.equal(mock.state.shows[0].mode, 'season');
	assert.equal(mock.state.shows[1].mode, 'episode');
	assert.equal((await put('shows/1/mode', { mode: 'default' })).status, 204);
	...
	assert.equal((await put('settings/reset-all', { mode: 'season' })).status, 204);
	assert.ok(mock.state.shows.every((show) => show.mode === 'season' && !show.mode_overridden));
	assert.equal(mock.state.shows[0].excluded, true);
	assert.equal((await put('shows/999/mode', { mode: 'season' })).status, 404);
	assert.equal((await fetch(`${base}/api/notifications/episode%3A1/test`, { method: 'POST', headers: { origin: base } })).status, 204);
	assert.equal((await fetch(`${base}/api/notifications/episode%3A1/test`, { method: 'POST', headers: { origin: 'https://other.example' } })).status, 403);

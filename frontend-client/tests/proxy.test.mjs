...
test('same-origin client bridge forwards API controls and handles failures', { timeout: 40_000 }, async (t) => {
	...
	for (let attempt = 0; attempt < 100; attempt++) {
		try { ready = (await fetch(`${base}/api/settings`)).ok; } catch { /* Vite is starting. */ }
		if (ready) break;
	...
	assert.deepEqual(await (await fetch(`${base}/api/settings`)).json(), { mode: 'episode' });
	assert.equal((await put('settings', { mode: 'season' })).status, 200);
	assert.equal(mock.state.mode, 'season');
	...
	assert.equal((await put('settings', { mode: 'invalid' })).status, 422);
	assert.equal((await put('settings', { mode: 'episode' }, 'https://other.example')).status, 403);
	assert.equal(mock.state.mode, 'season');

			...
			<section id="shows" class="card card-border">
				<div class="card-body">
					...
					{#if visibleShows.length}
						<div class="scroll-region"><table class="table"><thead><tr><th scope="col">Show</th><th scope="col">Status</th><th scope="col">Track</th></tr></thead><tbody>
							{#each pagedShows as show (show.id)}<tr><td class="show-name"><strong>{show.title}</strong></td><td><span class="badge" class:badge-ghost={!show.active || show.excluded} class:badge-success={show.active && !show.excluded} class:badge-soft={show.active && !show.excluded}>{!show.active ? 'Removed' : show.excluded ? 'Excluded' : 'Tracked'}</span></td><td><input type="checkbox" class="toggle toggle-primary" aria-label={`Track ${show.title}`} checked={!show.excluded} disabled={locked || !show.active} onchange={() => change(`show:${show.id}`, `shows/${show.id}/exclusion`, 'PUT', { excluded: !show.excluded }, `${show.title} ${show.excluded ? 'is now tracked' : 'is now excluded'}.`)} /></td></tr>{/each}
						</tbody></table></div>
